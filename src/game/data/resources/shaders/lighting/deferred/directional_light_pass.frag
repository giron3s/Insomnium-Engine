...
uniform vec3      u_LightPosition; // any_vertex-to-light vector, normalized
...
{ 
	...
	float kd = dot(N, u_LightPosition); // diffuse factor
	if ( kd > 0.0 ) {
		vec4 materialDiffuseColor = texture(u_diffuseMap, texCoord);
		o_color += u_LightColor * materialDiffuseColor * kd; // add diffuse term
		vec3 position = texture(u_positionMap, texCoord).xyz; // world position		
		...
		if ( ks > 0.0 )
		{
			...
			o_color += lightSpecularColor * materialSpecularColor * pow(ks, materialSpecularIntensity); // add specular term
		}
		// apply shadow
