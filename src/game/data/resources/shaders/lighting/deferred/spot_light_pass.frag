...
uniform vec3      u_LightDirection; // assumed normalized!
...
{ 
	...
	if ( kd > 0.0 ) {
		...
		float lightToSurfaceAngle = degrees(acos(dot(-L, u_LightDirection)));
		if ( lightToSurfaceAngle <= u_LightConeAngle )
		{
			float att = clamp(1.0 - dist*dist/(u_LightRadius*u_LightRadius), 0.0, 1.0);
			att *= att;
			att *= (1.0f - lightToSurfaceAngle / u_LightConeAngle);
			if ( att > 0.0 )
			{
