 ...
 *
 *               The FXAA render target applies Fast Approximate Anti-Aliasing from Timothy Lottes paper t Nvidia
 ********************************************************************************/
#pragma once

#include "graphic/fxaarendertarget.h"
...
namespace Framework
{
    ...
    private:
        ...
        {
            std::string lError;
            if (mShader->Load("anti-aliasing/fxaa_lottes", lError) == false)
            {
                CRASH("ERROR loading shader anti-aliasing/fxaa_lottes: %s\n", lError.c_str());
                return false;
            }
            return true;
        }
