...
#define __(call)                                                                                                                         \
    {                                                                                                                                    \
        glGetError(); /*dump previous error(s)*/                                                                                         \
        call;                                                                                                                            \
        GLuint error = glGetError();                                                                                                     \
        if (error != GL_NO_ERROR)                                                                                                        \
        {                                                                                                                                \
            fprintf(stderr, "ERROR 0x%x (%s) calling:\n\t%s\nin context:\n\t%s (%s:%d)\n\n", error, GL_ERROR_TO_STR(error), #call, __FILE__, __FILE__, __LINE__); \
        }                                                                                                                                \
    }
#else
    #define __(call) call
...
#define GL_CHECKPOINT()                                                                                                               \
    {                                                                                                                                 \
        GLuint error = glGetError();                                                                                                  \
        if (error != GL_NO_ERROR) {                                                                                                   \
            fprintf(stderr, "ERROR 0x%x (%s) in checkpoint for context:\n\t%s (%s:%d)\n\n", error, GL_ERROR_TO_STR(error), __FILE__, __FILE__, __LINE__); \
        }                                                                                                                             \
    }
