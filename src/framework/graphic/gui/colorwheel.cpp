/*
    src/UIColorWheel.cpp -- fancy analog widget to select a color value
...
namespace Framework
{
    namespace GUI
    {
        ...
        UIColorWheel::UIColorWheel(UIWidget *parent, const Color& rgb)
            : UIWidget(parent)
            , mDragRegion(None)
        {
            setColor(rgb);
        ...
        glm::ivec2 UIColorWheel::preferredSize(NVGcontext *) const
        {
            return { 100, 100. };
        }
        ...
        bool UIColorWheel::mouseButtonEvent(const glm::ivec2 &p, int button, bool down, int modifiers)
        {
        ...
        bool UIColorWheel::mouseDragEvent(const glm::ivec2 &p, const glm::ivec2 &, int, int)
        {
            return adjustPosition(p, mDragRegion) != None;
        }

        UIColorWheel::Region UIColorWheel::adjustPosition(const glm::ivec2 &p, Region consideredRegions)
        {
            float x = p.x - mPos.x,
                y = p.y - mPos.y,
                w = mSize.x,
        ...
        Color UIColorWheel::hue2rgb(float h) const {
            float s = 1., v = 1.;
        ...
        Color UIColorWheel::color() const
        {
            ...
            Color ret = (rgb * (1.0f - mWhite - mBlack)) + (black * mBlack) + (white * mWhite);
            ret.a() = 1.0f;
            return ret;
        }
        ...
        void UIColorWheel::setColor(const Color &rgb)
        {
            ...
            if (max == min) {
                ...
                M.topLeftCorner<3, 1>() = Vector3f(c.r(), c.g(), c.b());
                M(3, 0) = 1.;
                M.col(1) = Vector4f{ 0., 0., 0., 1. };
                M.col(2) = Vector4f{ 1., 1., 1., 1. };

                Vector4f rgb4{ r, g, b, 1.0f };
                Vector3f bary = M.colPivHouseholderQr().solve(rgb4);

                mBlack = isnan(bary[1]) ? 0 : bary[1];
                mWhite = isnan(bary[2]) ? 0 : bary[2];
            }
        }

        void UIColorWheel::Serialize(Json::Value& aSerializer) const
        {
            //Widget::save(s);
            //s.set("hue", mHue);
            //s.set("white", mWhite);
        ...
        void UIColorWheel::Deserialize(const Json::Value& aSerializer)
        {
            UIWidget::Deserialize(aSerializer);
            mHue   = aSerializer.get("hue", mHue).asFloat();
            mWhite = aSerializer.get("white", mWhite).asFloat();
            mBlack = aSerializer.get("black", mBlack).asFloat();
            mDragRegion = static_cast<Region>(aSerializer.get("drawregion",0).asInt());
        }
    }
}
